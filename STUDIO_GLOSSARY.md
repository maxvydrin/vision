...
| **User** | Person or system actor — including integrations and API clients — interacting with a Product. | Someone using Studio is a **Person** holding a tenant **Membership** (never "user" in canon). |
| **Workspace** | Only **Tool Workspace** — a container inside a specific Tooling System (Slack workspace, GitHub org). | **RETIRED as a Studio term (D-085, 2026-08-06)**: use **Project (root)**. Formerly Studio's working context bounded by one purpose; owns the Knowledge Graph. |
...
| **Organization** | The software-producing company being modeled. | Represented 1:1 by a **Tenant** (Studio's account object). |
...
| Fact store | — | **Knowledge Graph** *(one per tenant; objects scoped shared or tree)* | scoped `Object` graph | — |
...
| Person in Studio | Person / User | **Person** *(shared object)* + **Membership** *(tenant scope)* | `Principal` + WorkspaceMembership | User / Studio User |
| AI actor | AI Agent *(AI Estate)* | **Agent Definition** *(config)* / **Actor** *(who acts)* / Agent Role | `ActorRef` *(agent)* | AI Agent |
...
| ----------------------------- | ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| **Organization**              | Both    | **Org:** software-producing company or group of companies being modeled. **Studio:** represented by exactly one **Tenant** (1:1); there is no Organization object in the graph.                                                                                                                                                                                                                                | company; org §3.1 · repr §3.1                                                                |
...
| **Organizational Unit**       | Both    | Business unit, department, division or practice inside the Organization; hierarchical, contains child units and positions. **Does not contain Teams**: a Team is cross-functional and gathers people from several units.                                                                                                                                                                                                                                                                                                      | org unit, department                                                                         |
| **Person**                    | Both    | Real human being known to the Organization. In Studio a Person is a **shared** managed object: one per tenant, attached by the project trees that need it. It is never a grantee of roles — access hangs off the **Membership** that references it. | human, employee, contributor |
...
| **Team**                      | Both    | Stable group of people that builds, owns, operates, supports or governs work and assets. In Studio it is **one entity**, cross-functional: it gathers people from different Organizational Units and is never nested inside one. A team carries an **acquisition mode**, `authored` (membership managed in Studio) or `mirrored` (synced from a source system), and **role grants are allowed only on authored teams**, so a source system can never silently change access.                                                                                              | group · delivery team; repr §3.1                                               |
| **Team Membership**           | Both    | Dated membership of a Person in a Team. In Studio it is a **Membership** with team scope, which already carries the dates and allocation — not a separate entity. | team member relation |
...
| **User** | ⚠️ Both | **Org:** person or system actor — including integrations and API clients — interacting with a Product or affected by its behavior. **Studio:** someone using Studio is a **Person** holding a tenant **Membership**; "user" is not canon for that. The word "member" stays in prose for a person who holds one, but the entity is the Membership. | end user, operator |
...
| Term | Where | Definition | Aliases / Notes |
|---|---|---|---|
| **Project** | ⚠️ Both | *Unified — one entity, two faces (repr §3.1, 2026-07-12).* **Org:** bounded execution effort: feature delivery, research, migration, complex bugfix, customer implementation or remediation. **Studio:** the same managed object adopted as the effort container — the tenant's only hierarchical container (a **root project** owns its tree, D-085), includes objects **by reference**, adds inclusion scope, workflows, advancement; carries project-scoped Graph/Insight Views; does not own a graph. | delivery effort, initiative scope |
...
| **Tool Workspace** | ⚠️ Both | **Org:** collaboration or configuration container inside a specific Tooling System — a Slack workspace, Figma team, Notion workspace, Jira site, GitHub organization. The org model deliberately avoids a generic "Workspace". **Studio:** the Studio term **Workspace** is retired (D-085); Studio's working context is a **root Project** — see Part B. | org §3.6 · repr §3.1 |
...
| Term | Where | Definition | Aliases / Notes |
|---|---|---|---|
| **Policy** | Both | **Org:** normative organizational rule, expectation or requirement. **Studio:** arrives as a content-backed managed object, versioned, mirrored from a wiki or GRC tool or authored here. It states the commitment; the rules that enforce it are **Data Policy** and **Safety Constraint**, which cite it. | regulation, standard |
...
| **Membership** | Studio | Dated participation of a **Person** in something, and the record that carries the right to enter Studio. Its scope kind says in what: **tenant** (this person uses Studio) or **team** (this person is on that team, with dates and allocation) — one entity, the same fact at two scopes. **A tenant membership is always authored in Studio**: a source system may create and change team memberships, never tenant ones, so a directory can never hand out access. Holds Role Grants at a scope, directly or via teams. Client access (CLI, IDE plugins, MCP endpoints, API tokens) and locale are membership configuration, not domain entities. | Member, User Identity, Studio User |
| **Locale** | Studio | A member's language + formatting preferences (language, date/number/currency format, week-start, RTL) applied when Views render; member configuration, not a domain entity. | i18n; VISION §5.10 |
...
| **Role Grant** | Studio | The fact that a **membership, a team or a non-human actor** holds a Role **at a scope** — tenant or project (a project grant reaches the subtree, D-083); archiving a scope suspends its grants. **Never granted to a Person** (product invariant 15). | Role Assignment, scoped role |
...
| **Agent Role** | Studio | Specialized role an AI agent plays in an activity: analyst, implementer, reviewer, tester, summarizer. Distinct from access Role and org Job Role. | repr §6.2 || **Buyer (persona)** | Studio | The economic buyers for Studio — PM: CPO, Director of PM, PM; R&D: CTO, Chief Architect, VP Engineering, Head of Platform, Head of R&D. | Vision §4.2 |
| **Model Endpoint** | Studio | A provider-neutral point at which Studio calls a model, with context limit, price and data policy; its status describes the access, not the model. ⚠️ Not the org's **AI Model**, which is a model the organization has deployed. | model access, inference endpoint |
| **Agent Definition** | Studio | The configuration of an AI-assisted worker Studio runs: instructions, tools, model policy, budget. Definition, not execution; its runs are AI Runs. ⚠️ Not the org's **AI Agent**, which is an agent deployed in the organization. | agent config |
| **Tool** | Studio | A typed capability an agent, workflow or person may invoke through a registered endpoint; what a Model Routing Policy picks when the answer is not a model. | capability, function |
...
| Term | Where | Definition | Aliases / Notes |
|---|---|---|---|
| **Tenant** | Studio | The instance of Studio serving one Organization (1:1; may sit in a control-plane admin hierarchy, D-016). Contains root projects, org units, memberships, teams, roles, connectors; keeps the **control-plane registries**, the tenant-wide type registry, and the single **Knowledge Graph** in which shared objects are tenant-scoped. | account, instance |
| **Workspace** | ⚠️ Org only | **RETIRED as a Studio term (D-085, 2026-08-06).** Studio has one hierarchical container, the **Project**; a **root project** is the working context bounded by one purpose (typically a product line) and owns the tree-scoped objects. `workspace-scoped` → **`tree-scoped`**. **Org:** see **Tool Workspace** — a container inside a Tooling System (Slack workspace, GitHub org), unrelated. | — |
| **Inclusion** | Studio | Stored record of a project including a managed object by reference: who included it, when, manually or by rule. | scope membership |

## Graph, Objects & Sync

| Term | Where | Definition | Aliases / Notes |
|---|---|---|---|
| **Knowledge Graph** | Studio | The tenant's single home for the facts: every managed object and every relation, each carrying a **scope**. One graph per tenant; a project tree is a scope inside it, not the owner of a graph. A system model, not a UI — members see views. | graph |
| **Managed Object** | Studio | Studio's representation of one real entity. Carries a **scope**, fixed by its Object Type: **shared** (one object per tenant, project trees attach it — a Person, a repository, a commit, a build) or **tree** (belongs to exactly one project tree, and reaches another only as a copy — a PRD, a review). A shared object has one owner and one version history, so it may be edited through governed runs but never forked. | object, artifact (VISION), Object (kernel) |
| **Object Scope** | Studio | The property that says where a managed object or a relation lives: **shared** (tenant-wide, attached by project trees) or **tree** (belongs to one project tree, owned by its root). Declared by the Object Type and shipped by a kit, never chosen per object. | scope, tier |
| **Shared Object** | Studio | A managed object whose type is shared: one per tenant, one owner, one version history. Source-living things (a Person, a repository, a commit) and frozen-produced ones (a build, a release). Editable through governed runs; never forked. | tenant-scoped object |
| **Attachment** | Studio | The record that a project tree uses a shared object: who attached it, when, whether it is still active. Attaching is a tenant-scope right; detaching retires that tree's own relations to the object and touches no other tree. | attach, shared-object grant |
| **Local Overlay** | Studio | A project tree's own attributes on a shared object — *"in our line this repository is release-critical"*. Visible only in that tree and never changes the shared object. | local annotation |
| **Access Level** | Studio | How widely a shared object may be attached, set at tenant scope: **public** (any project tree) or **restricted** (an explicit list). A project tree cannot discover a shared object it has not been granted. | shared-object visibility |
| **Cross-tree Reference** | Studio | A deliberately weak, one-way pointer from an object in one project tree to an object in another, for cross-line dependencies a shared object cannot express. **Not a relation**: traversal stops at it, workflows and validators never see it, and nothing appears in the target tree. | cross-line pointer |
| **Object Type** | Studio | Registered type of managed object. Every domain-facing type **represents exactly one organization-model term**; a type may specialize another. Registry is tenant-wide; each container (tenant or project) activates its subset, and the effective ontology is computed along the path (D-082). Formerly: each workspace activated a subset. Provenance per type: built-in / kit / custom. | node type |
| **Relation** | Studio | Typed, directed connection between exactly two managed objects, with provenance and confidence. Carries a **scope** like an object: a **shared** relation joins two shared objects and is a mirrored source fact, visible wherever both endpoints are attached; a **tree** relation belongs to one project tree and is invisible to the others. Kinds: **imported** (fact from source), **asserted** (added by a person), **inferred** (proposed by Studio). | edge, link |
...
| **Graph View** | Studio | A view kind: scoped graph diagram over the tree's slice of the Knowledge Graph (a project's Graph View is project-scoped; the graph stays tree-level). | project subgraph |
| **Insight View** | Studio | A view kind: the window onto tree-level signals and delivery/cost metrics; project-scoped when in a project. | |
...
| **Object Mapping** | Studio | The type-level mapping for one connector: which external type becomes which Studio object type, which states map to which, and the field mappings under it. Versioned, so old imports stay explainable. | type mapping, schema mapping |
...
| **Recommendation** | Studio | Product label for a **prepared Action Run** surfaced from a Finding (*finding first, recommendation second*) — a proposed act awaiting approval; **not a distinct entity** (parallel to **Write-back**). | suggested change; repr §6.1 |
...
| **Workflow Library** | Studio | Catalog of workflows published to a container — predefined + custom; what a member browses and runs. Populated by kits. | Flow Library |
...
| **Context Package** | Studio | The selected set of objects, relations, content versions, immutable blobs, rules and evidence handed to an actor for one action or review — the unit of "what the model saw". Constrained by data policies and projections. | bound context |
...
| **Cost Budget** | Studio | Cap on AI spend at a scope (tenant / project / workflow / agent / model); alerts and blocks per policy. Ships in the `cf.ai` kit-set. | spend cap |
...
| Term | Where | Definition | Aliases / Notes |
|---|---|---|---|
| **Kit** | Studio | Package of reusable delivery knowledge that **defines a domain**: object & relation types (ontology contribution), templates, workflows, actions, validators, policies, reference architectures, Gears blocks. Origins: **shipped** or **authored** in the organization. Install registers types tenant-wide, activates them in the installing container. | Kit, Domain Kit |
...
| **Sample data** | Studio | Seed/demo data a kit ships to populate a project out of the box. | seed data |
...
| **Domain Dictionary** | Studio | Configurable vocabulary of a tenant or project; holds terminology overrides (the nearest project shadows the ones above — labels only). | |
| **Terminology Override** | Studio | Project- or tenant-specific **label** for a canonical concept (*Roadmap Item → "Research Plan Item"*). Changes labels, never semantics (invariant 11). | relabel |
| **Reference Catalog** | Studio | Editable named **set of allowed values** for a field (Work Item types, cost units, severities…); kit ships defaults, the container adds/disables, versioned + audited. Changes *values* only — not the label (Terminology Override) or the kind (Object Type). Value sets carrying logic stay fixed schema. Kernel: `configuration`-profile object. | lookup catalog, value set; repr §7 |
...
| **Conversation** | Studio | A durable, scoped session between a member and the Assistant: messages plus the context, runs and objects it produced. Distinct from a Discussion Thread, which is people commenting on an object. Anchored kinds: **Document Conversation** (a page or section) and **Inline Conversation** (a precise code, diff or checklist location). | session, chat |
| **Message** | Studio | One immutable turn in a Conversation: content, author, references, usage, tool activity. ⚠️ Not a Comment, which annotates an object. | turn |
| **Mention** | Studio | An explicit reference to a person, team or actor inside a comment, message or document; what turns a remark into someone's inbox item. | @-mention |
| **Review Request** | Studio | A routed ask for people or teams to review a pull request, document, architecture change or risk. The review itself is a Validator run; this is the routing and the state of the ask. | review routing |
| **Approval Request** | Studio | A routed ask for an approval decision, separate from the **Approval**, which is the recorded outcome. Keeping them apart makes "what is waiting on me" answerable. | approval routing |
