...
> **Index of:** software-organization-domain-model@0.9.4 · studio-representation-model@0.8.14 · 2026-07-13. **A term absent from a model is not canonical.**
...
| **Aliases / Notes** | Other names found across the docs (do not use in new work), deprecation tombstones, source pointers (`org §…` = software-organization-domain-model; `repr §…` = studio-representation-model). |
...
| **Insight** | Interpreted learning from evidence that may influence decisions. | Never merged (repr §2.2): the org Insight arrives as a managed object · the trust-ramp *read-only insight* stage renders as **Signals** · **Constructor Insight** is a product behind a connector. |
...
| **Responsibility Role**       | Studio  | Ownership/accountability role for a domain object: Product Owner, Tech Owner, Service Owner, Code Owner, Data Steward, Security Owner.                                                                                                                                                                                                                                                                     | ownership role                                                                               |
...
| **Insight** | ⚠️ Both | **Org:** interpreted learning from evidence that may influence strategy, product or commercial decisions; arrives in Studio as a managed object. **Studio:** the trust-ramp *read-only insight* stage renders as **Signals** in insight views · **Constructor Insight** is a separate product behind a connector. Three meanings, never merged (repr §2.2). | |
...
| **Work Item** | Both | Tracked unit of delivery or follow-up work; usually mirrored from a work tracker. Incidents, Risks and Findings are not Work Items — they may *create* Work Items. | delivery item |
...
| **Dependency** | Org | Relationship where one object, work item, team or external constraint affects another. | blocker |
| **Handoff** | Both | Transfer of responsibility, context or work output between people, teams or functions; in Studio also: passing one activity's outputs as another's inputs. | org §4 · Vision §8 |
...
| **AI Agent** | Both | Software actor that performs work using AI models under human accountability (attribution — invariant 8; ownership stays with people/teams). In Studio: an actor kind that runs the Skills a kit ships. | agent |
...
| **Finding** | Org | Observed issue, gap, non-compliance, vulnerability or problem. | gap, issue |
...
| Term | Where | Definition | Aliases / Notes |
|---|---|---|---|
| **Constructor Studio** | Studio | AI-native, open-core **control plane** that mirrors a team's existing SDLC tools into one governed graph and moves work forward with validated, human-approved actions — across the SCLC, not just coding. | Studio |
...
| Term | Where | Definition | Aliases / Notes |
|---|---|---|---|
| **Actor** | Studio | Anything that performs a transformation in the lifecycle: Human, AI Agent, external system, automated pipeline. | Vision §7.1 |
...
| **Role** | ⚠️ Both | **Studio:** named permission set from the tenant's role catalog, granted to members and teams via Role Grants. **Org:** for the organizational sense use **Job Role** (work role) or **Responsibility Role** (ownership) — never merged (repr §2.2). | Access Role, permission set |
...
| **User Projection** | Studio | The effective subset one member can see, computed from their role grants (direct + via teams). Objects outside it render as **locked stubs**; access never propagates along relations. | effective visibility |
...
| **Buyer (persona)** | Studio | The economic buyers for Studio — PM: CPO, Director of PM, PM; R&D: CTO, Chief Architect, VP Engineering, Head of Platform, Head of R&D. | Vision §6.2 |
...
| Term | Where | Definition | Aliases / Notes |
|---|---|---|---|
| **Studio Tenant** | Studio | The instance of Studio serving one Organization (1:1). Contains workspaces, members, teams, connectors; keeps the **tenant-wide identity registry** — one identity per real entity. Commercial packaging is tenant configuration, not domain entities. | account, tenant |
...
| **Managed Object** | Studio | Studio's representation of one real entity. One object per tenant; workspaces include it by reference, never copy. Carries one acquisition mode (**mirrored / linked / authored**); may be **content-backed** (owns versioned content — informally, a document); records provenance, confidence, sync state. Typed by an Object Type. | Graph Object, Node |
...
| **Signal** | Studio | Computed observation over the graph — gap, drift, contradiction, staleness; never authored (invariant 14). Accepting one for work *authors* a managed object (e.g., an Opportunity) linked to its signal. | gap signal |
...
| Term | Where | Definition | Aliases / Notes |
|---|---|---|---|
| **SCLC (Software Construction Lifecycle)** | Studio | The full lifecycle Studio champions: from Intent through construction and operation to learning and evolution — wider than SDLC. 3 phases (Plan → Build → Operate), 14 stages. | Vision §8 |
| **SDLC** | Studio | The traditional development lifecycle focused on *creating* software; Studio adapts to the company's SDLC and extends it into an SCLC. | |
| **Reference SCLC** | Studio | The example lifecycle Studio ships (Intent → … → Evolution); usable as-is or customizable; delivered by the SDLC Kit. | |
...
| **Input / Output** | Studio | The artifact(s) an Activity consumes / produces; one activity's Output can feed another's Input. | Vision §8 |
...
| **Automation** | Studio | Rule that reacts to events, sync results or schedules; triggers workflows or actions. | trigger rule |
...
| **AI Run** | Studio | One execution by a model or agent: model used, tokens, runtime, **cost**. The atomic unit under every AI-spend surface; attributed to an actor. | model call |
...
| **SDLC Kit** | Studio | The open-source base/default kit; implements the Reference SCLC. | |
...
| **Domain Profile** | Studio | Package of mappings, terminology, validators, workflows and templates for a target domain or operating model. | |
...
| Term | Where | Definition | Aliases / Notes |
|---|---|---|---|
| **Collaboration** | Studio | The cross-cutting sharing + coordination layer: reviews · comments · handoffs · activity feed · sharing. | Vision §9 |
...
| **Guardrails & Quality Gates** | Studio | Shared standards defined at team/org level, with per-project enforcement. | Vision §9 |
...
| **Opportunity Statement** | Studio | The Intent-stage output naming a market/customer opportunity worth pursuing. | Vision §8 |
