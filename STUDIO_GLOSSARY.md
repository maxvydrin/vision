...
| Fact store | — | **Knowledge Graph** *(one per tenant; objects scoped shared or workspace)* | scoped `Object` graph | — |
...
| **Person**                    | Both    | Real human being known to the Organization. In Studio a Person is a **shared** managed object: one per tenant, attached by the workspaces that need it. It is never a grantee of roles — access hangs off the **Membership** that references it. | human, employee, contributor |
...
| **Role Grant** | Studio | The fact that a **membership, a team or a non-human actor** holds a Role **at a scope** — tenant, workspace or project; archiving a scope suspends its grants. **Never granted to a Person** (product invariant 15). | Role Assignment, scoped role |
...
| Term | Where | Definition | Aliases / Notes |
|---|---|---|---|
| **Knowledge Graph** | Studio | The tenant's single home for the facts: every managed object and every relation, each carrying a **scope**. One graph per tenant; a workspace is a scope inside it, not the owner of a graph. A system model, not a UI — members see views. | graph |
| **Managed Object** | Studio | Studio's representation of one real entity. Carries a **scope**, fixed by its Object Type: **shared** (one object per tenant, workspaces attach it — a Person, a repository, a commit, a build) or **workspace** (belongs to exactly one, and reaches another only as a copy — a PRD, a review). A shared object has one owner and one version history, so it may be edited through governed runs but never forked. | object, artifact (VISION), Object (kernel) |
| **Object Scope** | Studio | The property that says where a managed object or a relation lives: **shared** (tenant-wide, attached by workspaces) or **workspace** (belongs to one). Declared by the Object Type and shipped by a kit, never chosen per object. | scope, tier |
...
| **Attachment** | Studio | The record that a workspace uses a shared object: who attached it, when, whether it is still active. Attaching is a tenant-scope right; detaching retires that workspace's own relations to the object and touches no other workspace. | attach, shared-object grant |
| **Local Overlay** | Studio | A workspace's own attributes on a shared object — *"in our line this repository is release-critical"*. Visible only in that workspace and never changes the shared object. | local annotation |
| **Access Level** | Studio | How widely a shared object may be attached, set at tenant scope: **public** (any workspace) or **restricted** (an explicit list). A workspace cannot discover a shared object it has not been granted. | shared-object visibility |
| **Cross-workspace Reference** | Studio | A deliberately weak, one-way pointer from an object in one workspace to an object in another, for cross-line dependencies a shared object cannot express. **Not a relation**: traversal stops at it, workflows and validators never see it, and nothing appears in the target workspace. | cross-line pointer |
