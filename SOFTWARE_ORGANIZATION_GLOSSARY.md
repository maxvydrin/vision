...
Index of software-organization-domain-model@0.9.1, generated 2026-07-13.
...
| **Employee** | Person employed by a Legal Entity. | deprecated — not an entity in model@0.9.1; only **Employment** (the reified relationship, §3.1) is modeled — record such distinctions on it |
| **Contractor** | Person engaged under contract rather than employment. | deprecated — not an entity in model@0.9.1; only **Employment** (the reified relationship, §3.1) is modeled — record such distinctions on it |
| **External Collaborator** | Person outside the Organization who participates in work. | deprecated — not an entity in model@0.9.1; only **Employment** (the reified relationship, §3.1) is modeled — record such distinctions on it |
...
| **Vision** | Desired future state and product/business ambition. A Vision has exactly one scope target — Organization, Product Line or Product — with at most one active Vision per target (one Product, one Vision). | north star; Company Vision = scope Organization, Product Vision = scope Product/Line |
| **Product Vision** | A Vision scoped to a single Product or Product Line — one Product, one Vision. Not a separate entity: the **Vision** term with scope = Product/Line. | see Vision |
...
| **Investment Direction** | Chosen direction for allocating funding, capacity or attention. | renamed from Investment (model@0.9.1 §3.2) |
...
| **Budget** | Approved funding envelope for a period and scope: organizational unit, strategic initiative, project or team. | funding envelope (model@0.9.1 §3.2) |
| **Spend Record** | Recorded actual cost — people, vendors, infrastructure, tooling or AI model usage — compared against Budgets. | actual spend (model@0.9.1 §3.2) |
...
| **Roadmap Type** | Classification of a Roadmap: company, product, platform, engineering, technology or governance. | model@0.9.1 §3.2 |
| **Roadmap Scope** | Domain object or area a Roadmap is about, such as Organization, Product Line, Product, Product Capability, Software System or Team. | model@0.9.1 §3.2 |
| **Roadmap Horizon** | Planning time window for a Roadmap: quarter, half-year, year or custom period. | model@0.9.1 §3.2 |
| **Product Roadmap** | A Roadmap with type = product, scoped to a Product Line or Product. Not a separate entity: the **Roadmap** term with a type + scope. | see Roadmap |
...
| **Design Artifact** | Design output specifying user experience or interface for a feature or capability: wireframe, prototype or design specification. | model@0.9.1 §3.4 |
...
| **Commercial Customer Relationship** | Ongoing relationship between the organization and a customer account across buying, usage, support and renewal. | model@0.9.1 §3.5 |
| **Deal** | Sales opportunity with a customer account, moving through pipeline stages toward an agreement. | model@0.9.1 §3.5 |
| **Win/Loss Record** | Recorded outcome and reasons of a closed deal. | model@0.9.1 §3.5 |
| **Commercial Plan** | Packaged commercial offer — often called a plan or tier — defining available capabilities, limits, pricing or service level. | plan, tier (model@0.9.1 §3.5) |
| **Pricing** | Rules or model for how product value is monetized. | model@0.9.1 §3.5 |
| **Entitlement** | Right granted to a customer, account or user to access a product capability, plan, limit or service. | model@0.9.1 §3.5 |
...
| **Subscription** | Ongoing commercial relationship granting access to a product, plan or service for a period. | model@0.9.1 §3.5 |
| **Renewal** | Commercial event or process of extending, changing or ending an existing customer relationship. | model@0.9.1 §3.5 |
| **Support Case** | Tracked customer support request or issue with its own lifecycle and response commitments. | model@0.9.1 §3.5 |
| **Customer Health** | Assessed state of a customer relationship, derived from usage, support cases, incidents and renewals. | model@0.9.1 §3.5 |
| **Usage** | Measured product consumption, adoption or activity by customers or accounts. | model@0.9.1 §3.5 |
...
| **Market** | External arena in which the organization competes, sells, learns or seeks adoption. | model@0.9.1 §3.3 |
| **Market Segment** | Target market, customer group or context in which Products compete. | segment |
| **Persona** | Representative user, buyer, customer or stakeholder type used to reason about needs and behavior. | model@0.9.1 §3.3 |
...
| **Research Evidence** | Observed or collected evidence from research, analysis, interviews, experiments or market monitoring. | model@0.9.1 §3.3 |
| **Insight** | Interpreted learning from evidence that may influence strategy, product or commercial decisions. | model@0.9.1 §3.3 |
...
| **Process** | Repeatable way of performing work or making decisions. | model@0.9.1 §4 |
| **Activity** | Bounded action or step within a process, project or workstream. | model@0.9.1 §4 |
...
| **Work Item Type** | Classification of a Work Item; controlled vocabulary: epic, user story, task, bug, spike, chore, incident follow-up, remediation task. | values, not separate entities (model@0.9.1 §4) |
...
| **Epic** | Large body of work decomposed into User Stories or Tasks. | deprecated — **Work Item Type** value, not an entity (model@0.9.1 §4) |
| **User Story** | User-centered delivery item with acceptance criteria. | deprecated — **Work Item Type** value, not an entity (model@0.9.1 §4) |
| **Task** | Concrete unit of work. | deprecated — **Work Item Type** value, not an entity (model@0.9.1 §4) |
| **Bug** | Defect or incorrect behavior requiring correction. | deprecated — **Work Item Type** value, not an entity (model@0.9.1 §4) |
| **Remediation Task** | Work created to mitigate Risk, resolve Finding or prevent Incident recurrence. | deprecated — **Work Item Type** value, not an entity (model@0.9.1 §4) |
| **Dependency** | Relationship where one object, work item, team or external constraint affects another. | blocker; renamed from Work Dependency (model@0.9.1 §4) |
| **Handoff** | Transfer of responsibility, context or work output between people, teams or functions. | model@0.9.1 §4 |
| **Outcome** | Result or effect the work is intended to create or has actually created. | model@0.9.1 §4 |
...
| **Tooling System** | Software system used by the organization to plan, build, test, run, support or govern work. | renamed from Tool; internally built or vendor-provided (model@0.9.1 §3.6) |
| **Tool Workspace** | Collaboration or configuration container inside a specific Tooling System, such as a Slack workspace, Figma team, Notion workspace, Jira project space or GitHub organization. | model@0.9.1 §3.6 |
| **Technology Stack** | Set of technologies, tooling systems, frameworks, languages and platforms used by the organization. | stack |
| **AI Model** | AI/ML model used by products or tooling — vendor-provided or internally built. | model@0.9.1 §3.6 |
| **Prompt Asset** | Managed prompt, prompt template or agent instruction used with AI models. | model@0.9.1 §3.6 |
| **Eval Run** | Evaluation of an AI model, prompt or AI-backed feature against defined criteria. | model@0.9.1 §3.6 |
| **AI Agent** | Software actor that performs work using AI models under human accountability. | accountable ownership stays with people and teams (model@0.9.1 §3.6) |
...
| **Code Review** | Review activity or record evaluating a Pull Request or code change before merge or release. | model@0.9.1 §3.7 |
...
| **Change Record** | Record that connects a delivered change to its scope, evidence, decision and deployment context. | model@0.9.1 §3.7 |
...
| **Telemetry Signal** | Observed runtime signal such as log, trace, metric, event or health check. | model@0.9.1 §3.8 |
...
| **Problem** | Underlying or recurring cause behind incidents, defects or operational instability. | model@0.9.1 §3.8 |
...
| **Service Health** | Current or historical view of whether a service is operating within expected thresholds. | model@0.9.1 §3.8 |
...
| **External Service** | Service operated by an external party and used by the organization. | model@0.9.1 §3.10 |
| **Technology Dependency** | Dependency on a technology, framework, platform, runtime or standard outside direct product ownership. | model@0.9.1 §3.10 |
| **Supply-Chain Risk** | Risk created by external components, vendors, services, licenses or delivery dependencies. | model@0.9.1 §3.10 |
