...
related:
  ...
  - "project-entity-mapping-and-kit-profiles"
---
...
- [10. Worked Example And Scenarios](#10-worked-example-and-scenarios)
  ...
  - [10.4 A portfolio lead looks across lines (the per-tree boundary, honestly)](#104-a-portfolio-lead-looks-across-lines-the-per-tree-boundary-honestly)
...
- [Appendix D. Working across project trees — requirements (R1–R10)](#appendix-d-working-across-project-trees--requirements-r1r10)
...
> An **Organization** gets a **Tenant**. Connectors pull records from the organization's tools; identity mapping turns them into **managed objects** in the tenant's single **Knowledge Graph**. Every object carries a **scope** set by its type: **shared** things (a person, a repository, a commit, a build) exist once per tenant and project trees *attach* them; **tree** things (a PRD, a review) belong to one project tree and reach another only as a copy. A **project tree** is Studio's working context for one purpose (typically a product line): its root project owns the objects, plus the shared ones the tree has attached, together with the **relations** among them. A **Project** — an effort that is itself an object in the graph — gathers the objects it touches and drives them to an outcome; projects **nest into a tree**, so an umbrella effort holds child efforts and passes its configuration down without ever passing data, scope or ownership. **Members** work through **views**, seeing exactly what their **roles** allow. On top of the mirror, **workflows** run **actions**: Studio computes **findings** — gaps, drift, contradictions — prepares recommendations, validates candidates against **quality gates** attaching **evidence** — a validator may be a rule, a test, a model or a **person reviewing** — and, only after **approval**, writes back to the source tools. **Kits** package the domain knowledge all of this runs on — object types, templates, workflows, validators, **Gears** building blocks, and a **Project Profile** saying what a project of that domain requires — kits may extend one another, and any container installs them: the tenant, or a project and its subtree. *(How you adapt a kit to your organization — rename, re-value, re-type, re-structure — is the four-rung map in §7.)*
...
flowchart LR
  ...
  Tenant -->|"contains"| Project
  Tenant -->|"connects via"| Connector
  Tenant -->|"owns"| Graph
  Graph -->|"contains (shared or tree-scoped)"| MO
...
**Running example, used throughout:** *Acronis runs Studio. Its tenant connects Jira, GitLab and the HRIS (human-resources system). The "Backup & Recovery" product line — with its product **Cyber Protect** — is a root **Project**; inside it, the "Clean Restore" Project gathers everything that effort touches: the competitor signal, the roadmap item, the specs, the repositories and the pull requests. Each employee exists in Jira, GitLab and the HRIS — three source records; identity mapping assembles them into one Person object — one page, one history, three provenance trails. Person is a **shared** object (§3.1.1), so that is one object for the whole tenant, and every project tree that needs the engineer attaches the same one.*
...
**Retired terms.** **Workspace** is retired (D-085, 2026-08-06): Studio has one hierarchical container, the **Project**, and a **root project** carries what Workspace used to carry — the working context bounded by one purpose, and ownership of everything authored inside it. `workspace-scoped` is now **`tree-scoped`**, `Cross-workspace Reference` is now **`Cross-tree Reference`**, and "per workspace" reads "per project tree". The word survives only where an upstream document quotes it (VISION, the kernel contract) and must not re-enter this model. It is **not** a synonym for the tenant admin hierarchy (§3.1.1), which is a different axis.

## 3. Studio's World

**One discipline runs through the whole model: definition vs instance.** Every working thing in Studio exists twice: as a **definition** — configuration, chosen and shipped *as data* by kits and admins (invariant 13) — and as an **instance** — what actually happened. Think blueprint vs part: an Object Type says what a Requirement *is*; a Managed Object is requirement R-12. A Workflow is the pipeline's definition; a Workflow Run is yesterday's execution of it. A Template is the blank; an authored object is the filled-in document. The CLC definition (the kit's configured lifecycle) is the blueprint; the applied stage state is where work stands right now. The payoff: **adapting Studio to an organization means changing the left column — install or edit a kit — never the product itself.** Durable configuration has three owners — tenant control config, **project-inherited config** (a closed list: naming convention, template set, Project Profile, required creation fields, validator and lamp sets; inherited down the project tree, configuration only, never data or scope, invariant 17), and **kit configuration, which is a container-owned graph object with immutable versions** (templates, lifecycle definitions, rule sets), authoritative only as an exact version (§6.4).
...
**How to read the `Facing` column** (used in every entity table). **Cardinality echoes:** entity rows repeat, in parentheses, the numbers readers ask first — how many projects, roles; the normative source stays the relationship catalog (§8.1). 
...
**Two layers, one tenant.** Everything in Studio lives in exactly one of two layers:

1. **Control-plane record** — Tenant, Membership, Team, Role, Permission, Data Policy, Safety Constraint, Connector, Kit, Attachment, Audit Entry. These govern access and configuration; they are **not part of the knowledge graph**, and **authorization flows through them only — never through the graph** (invariant 15). A tenant-level change (a role, a policy) reaches every project tree directly, with no graph involved.
2. **Graph object** — a kit-typed managed object, shared or tree-scoped: the domain data (§3.2).
3. There is no third layer any more. A **Person** is an ordinary shared graph object, and the control-plane record that points at it is a **Membership** (§3.1). Stand-ins are gone: authorization reads the membership, never the Person.

These two layers place *where a thing lives*; a graph object is further placed by its **scope** (§3.1.1), and the per-**field** refinement — who may write each field — is **§3.4**.

### 3.1 Containers and people

| Term          | Meaning                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Primary relationships                                                                                                                                                                                                                                                                                                                                                                                                                                               | Facing       |
| ------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------ |
| **Tenant**    | The instance of Studio serving one Organization (**1:1** — unchanged). *Acronis = one tenant.* Commercial packaging (subscription plan, account policies) is tenant configuration, not domain entities. A tenant may sit under a **parent tenant** in a control-plane **admin hierarchy**: a service provider's parent tenant administers N child tenants, each still 1:1 with its own organization — the hierarchy governs *management*, never data (each tenant's objects stay isolated). Kernel already supports this (`parentTenantId`). | Contains root projects, org units, memberships, teams, roles and connectors (any number of each; the tenant itself is one per organization); keeps the **control-plane registries** — memberships, teams, roles, policies, connectors, kits, audit — the tenant-wide **type** registry (invariant 13), and the single **Knowledge Graph**, in which shared objects are tenant-scoped and tree objects belong to their project tree. **May administer child tenants** (`Tenant → administers → Tenant`, 1:N, control-plane only, deployment-gated). | Admin-facing |
| **Project**   | Studio's **only container of work** and its effort container — **itself a managed object of type Project**: usually *authored* in Studio, sometimes mirrored from a tracker and **adopted**. A **root project** (no parent) is also Studio's **working context bounded by one purpose**, typically a product line: it owns the tree-scoped objects and attaches the shared ones the tree is granted (§3.1.1). There is no separate working-context entity above it (D-085). Roughly a tracker's epic or initiative — a goal, the objects that effort touches, an outcome; Studio adds the workbench: inclusion scope, workflows, progress. *"Clean Restore" gathers the gap, the roadmap item, the repos and the PRs of one push.* **Projects nest** into a strict tree: an umbrella project ("BKP") holds child projects, configuration inherits down it and data, scope and ownership never do (invariant 17). A project may activate its own kits, so a subtree may run its own CLC (§7). A root project **cannot be deleted or archived while it owns objects**, and its identity is the tree's scope handle (invariant 6b).                                                                                                                             | Is a managed object (type Project); belongs to the tenant when it is a root, otherwise to **exactly one** parent project (a tenant holds **any number** of root projects); **may contain child projects** (at most one parent, invariant 17); a **root** owns the tree's tree-scoped objects and attaches shared ones (§3.1.1); includes managed objects by reference (an object may belong to several projects at once); *advances* the managed objects it exists for (a Roadmap Item, an epic work item).                                                                                                                                                                | User-facing  |
| **Membership** | Dated participation of a **Person** in something, and the record that carries the right to enter Studio. Its `scope kind` says in what: **tenant** (this person uses Studio) or **team** (this person is on that team, with dates and allocation). One entity, because both are the same fact at different scopes. **A tenant membership is always authored in Studio** — a source system may create and change team memberships, never tenant ones, so a change in a directory can never hand anyone access. Client access — CLI, IDE plugins, MCP endpoints, API tokens — and locale are **membership configuration, not domain entities**. | A control-plane citizen when its scope is the tenant. References exactly one Person (§3.2, shared); holds **any number** of Role Grants, each at one scope, directly or via teams. **A person works in any number of project trees and projects at once** — participation *is* the role grant at that scope; there is no separate "project membership" record. | User-facing |
| **Team**      | A Studio **group of people**: a grantee of roles and a unit of organizing work. Cross-functional by design, it gathers people from different Org Units, so a person sits in **one** Org Unit and in **any number** of teams. Every team records where its membership comes from, the **acquisition mode** (§3.2): `authored` (managed in Studio) or `mirrored` (synced from a source system). **Role grants are allowed only on `authored` teams**, so a change in a directory or a Git group can never silently change access in Studio. A mirrored team may be *adopted*: its membership is copied once, after which it is authored. | Belongs to the tenant (any number of teams); has members (a member may belong to many teams); granted roles at a scope, only when `authored`; membership changes are audit-logged. *(Realization: the kernel has no group grantee, a team materializes as per-principal grants applied by a governed Worker, one audit entry per change.)* | User-facing  |

#### 3.1.1 Working across project trees

**One graph, two scopes, defined once here** *(referenced throughout the doc)*: the tenant has **one Knowledge Graph**, and every object and every relation in it carries a **scope**. A **shared** object lives at tenant level and project trees *attach* it; a **tree** object belongs to exactly one **project tree**, owned by that tree's **root project**. Scope is not chosen per object: it is declared **by the object's type**, and a kit declares it (§3.2). Where the doc says "a shared object," it means an object whose type is declared shared.

**Why the tree and not a separate container.** Studio has one hierarchical container, the Project (§3.1, D-085). A root project is therefore both the working context bounded by one purpose and the owner of everything authored inside it: an umbrella project "BKP" holds the child projects, owns their authored content, and attaches the shared objects the tree is granted. The scope handle is the **root project's identity**, which is stable and never reused; a root **cannot be deleted or archived while it owns objects**, and a project can never be moved between trees (invariant 6b, D-080).

Isolation therefore rests on **filtering, not on structure**: there is no graph above the tree to wall things off, so every traversal, search and API path is bounded by the viewer's projection, and no unbounded path to the graph exists (invariants 6a–6c). This is a deliberate trade: one graph is simpler to reason about and to implement, and the guarantee moves into rules that must be owned and tested.

**What is shared, and what stays local.** Things that live in a source and that Studio mirrors rather than authors are shared: a **Person**, a repository, a commit, a branch, a build, a release, a software system, a service. One object per tenant, one history, one owner. Editable authored content — a PRD, a competitor review, a decision — stays **tree**-scoped: it belongs to the tree that authored it and reaches another tree only as an independent copy, because two editors of "the same" document would diverge.

**A shared object may still be edited**, and that is not a contradiction: because it is *one* object with *one* version history, there is nothing to fork. Edits go through a governed action run under the object's owner and permissions (§6.1), exactly like any other governed change. What is forbidden is not editing but *divergence*.

**A project tree may add its own reading on top of a shared object** — a Local Overlay (§3.2): attributes that belong to that tree, invisible to others, that never change the shared object itself.

The requirements (R1–R10), and which clauses still need a kernel change, are in **Appendix D**.


**Studio supports service-provider / MSP topologies** (VISION §6.4 "Service providers: Core focus"): a **parent tenant administers N child tenants**, each still 1:1 with its own organization — one provider console over many client tenants, with the provider able to push its kits down the hierarchy (§7). The **tenant admin hierarchy** (§3.1 Tenant) that enables this is a *different axis* from cross-tree sharing and must not be confused with it: it is **control-plane only** — the hierarchy governs *management*, never data — and it creates **no** shared objects, no shared identity and no cross-tenant graph. Sharing is a **within-tenant** mechanism only: a shared object is shared across the project trees of one tenant and never across tenants. The only sanctioned cross-tenant move is a parent **distributing a kit** (definition, copy-only) down the hierarchy (§7).

### 3.2 Objects and relations

| Term                | Meaning                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Primary relationships                                                                                                                            | Facing        |
| ------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ | ------------- |
| **Knowledge Graph** | The tenant's single home for the facts: every managed object and every relation, each carrying a **scope** (§3.1.1). One graph per tenant, not per tree — a project tree is a *scope within it*, not a container of its own graph. A graph is a **system model, not a UI**: members see views; a graph diagram is just one view. | Belongs 1:1 to a tenant; queried by views, workflows and automation, always through the viewer's projection. | System-facing |
| **Managed Object**  | Studio's representation of one real entity. It **is the kernel `Object` metatype seen at the product layer**, with acquisition/provenance/sync added; the name keeps "Managed" because bare "object" is used generically throughout. *(STUDIO_VISION calls this "artifact"; kernel/ARCH say "Object" — same entity, see glossary alias table.)* Every object carries a **scope**: **shared** (one per tenant, project trees attach it) or **tree** (belongs to exactly one project tree, owned by its root). Scope comes from the object's **type**, never from a per-object choice. A shared object has one owner and one version history, so editing it is allowed and forking it is not (§3.1.1). Everything else it carries — below this table. | Typed by an Object Type, which fixes its scope; assembled from source records via identity mapping (authored objects have none); attached to project trees when shared, owned by one tree's root when not; included into projects. | User-facing |
| **Relation**        | A typed, directed link between exactly two managed objects — *"PR **resolves** Work Item"*. Like an object it carries a **scope**. A **shared** relation joins two shared objects and is a mirrored source fact (a commit belongs to a repository); it is visible to every project tree that has both endpoints attached. A **tree** relation belongs to exactly one project tree and may join that tree's own objects, an object and an attached shared one, or even two shared ones — the last being that tree's own reading, invisible to others. Carries an **origin** (imported, or asserted / inferred) plus the same provenance and confidence as an object. | Instance of a Relation Type; scoped shared or to one project tree. | User-facing |
| **Object Type**     | The blueprint of one kind of object: what a Requirement, a PR or a Release *is* — its expected attributes, which relations it may enter, and **whether its objects are shared or tree-scoped** (§3.1.1). Four rules: every domain-facing type stands for **exactly one organization-model term**; a type may **specialize** another (Metric-by-domain, Decision subtypes); a type declares its **scope**, and a kit ships that declaration; **one registry per tenant, each container activates its subset** — the tenant or a project, with the container's *effective ontology* **computed along its path** (invariant 13). Types come from three suppliers — **built-in**, **kit install**, **custom** — recorded per type. Customers may extend kit types with governed external attributes without forking the type — declared by kits, activation-gated, never weakening the base schema (kernel contract). A typed attribute may be **constrained by a Reference Catalog** (§7) — an editable value set, not a fixed schema enum — when its allowed values are an operational choice. | Types managed objects and fixes their scope; defines expected attributes and allowed relation types; registered at the tenant; activated per container (tenant / project). | Admin-facing |
| **Relation Type**   | Registered kind of relation (`realized_by`, `implements`, `depends_on`, `has_owner`, `supports`…). Mirrors the organization model's primary relationships. Registered tenant-wide and activated per container, like object types.                                                                                                                                                                                                                                                                                                              | Constrains which object types it may connect.                                                                                                    | Admin-facing  |
| **Attachment**      | The record that a project tree uses a shared object: who attached it, when, and whether it is still active. Attaching is a **tenant-scope right** (§5) — a tree cannot help itself to a shared object it was not granted. Detaching retires that tree's own relations to the object and touches no other tree. | Pairs one project tree (by its root) with one shared object; created and revoked at tenant scope; audit-logged. | Admin-facing |
| **Local Overlay**   | A project tree's own reading of a shared object: attributes that belong to that tree alone. *"In our line this repository is release-critical."* Invisible to other trees, and it never changes the shared object. | Belongs to one project tree and one shared object; edited per that tree's grants. | User-facing |
| **Cross-tree Reference** | A deliberately weak pointer from an object in one project tree to an object in another, for cross-line dependencies a shared object cannot express (*"our release depends on their component"*). **It is not a relation**: traversal always stops at it, workflows and validators do not see it, the far side renders at its type's Data Policy visibility level, and nothing appears in the target tree. | Belongs to the tree that created it; one-way; never traversed (invariant 5c). | User-facing |

**What every managed object carries:**

- **Scope** — **shared** (one object per tenant, project trees attach it) or **tree** (belongs to exactly one project tree, owned by its root). Fixed by the object's type, never chosen per object.
...
| Term             | Meaning                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Primary relationships                                                                                                                                                                                        | Facing                                                                                |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------- |
| **View**         | Saved way of selecting and presenting objects: list, board, table, timeline, dashboard, graph diagram. Recurring kinds: *object view* (one object + its neighborhood), *traceability view* (intent → requirement → code → test → release → feedback), *review view* (candidates, validation results, write-backs pending approval), **graph view** (a scoped diagram over the tree's slice of the Knowledge Graph), **insight view** (the window onto tree-level signals and delivery metrics). A project's **Graph View** and **Insight View** are exactly this — project-scoped views; the graph and the signals themselves stay at tree level. | Belongs to a project (a root or any node) **or** a member (exactly one); selects by query; always renders through the viewer's projection.                                                                           | User-facing                                                                           |
...
*The same engineer appears as a Jira user, a GitLab account and an HRIS row — three source records. Identity mapping declares them **one Person object for the tenant** — Person is a shared type (§3.1.1), so a second project tree importing the same sources attaches that same object rather than minting its own.*
...
| **Source Record** | One original record as fetched (an issue, a repo, an employee row). | Maps to **at most one managed object per project tree** (a record imported into N trees yields N objects); superseded by newer fetches. | System-facing |
...
| **Identity Mapping** | Rule or confirmed fact that several source records are the same real entity. Studio proposes matches (e-mail, linked accounts, names); an administrator confirms uncertain ones and can split wrong merges; every merge/split is audit-logged. | Merges source records into one managed object, at the scope its type declares: one per tenant for a shared type, one per project tree otherwise. Object identities themselves are never merged, split or reused. | Admin-facing |
| **Object Mapping** | The type-level mapping for one connector: which external type becomes which Studio object type, which of its **states** map to which, and the field mappings that go with it. Versioned, because a mapping changes while old imports must stay explainable. Without it nothing says what a Jira issue of type Bug becomes, or what its "In Progress" means here. | Configured per connector; targets one object type; contains field mappings; versioned and audited. | Admin-facing |
| **Field Mapping** | Mapping between source-record fields and managed-object attributes. | Configured per connector / object type; belongs to an Object Mapping. | Admin-facing |
...
Admin configures Connectors (Jira, GitLab, Confluence, HRIS)
  ...
  -> Managed Objects materialize in each importing project tree as that tree's
     own objects; a Source Link records provenance per object
  -> imported Relations materialize per tree — both endpoints in the same graph
  -> Sync State and Conflicts surface gaps and stale links; conflicts resolve per governance rules
```

## 5. Who Sees What

Access model: **RBAC** (role-based access control). Permissions attach to roles; roles are granted to **memberships, teams and non-human actors** (an AI agent, an external system, a pipeline); nothing is ever attached to an individual object. **A role is never granted to a Person**: a Person is a graph object that a source system creates and changes, so granting on it would let a directory hand out access. Rights hang off the membership, which is always authored in Studio (§3.1). Attaching a shared object to a project tree is likewise a **tenant-scope right**. Attribute-level restriction, explicit deny and legal walls (**ABAC**) are a **deferred enterprise variant** (D-041), not part of the phase-1 base model.

**A project-scope grant reaches the project's whole subtree, always** (invariant 17, D-083): granting a role on an umbrella project grants it on every project under it, and there is no per-grant opt-out. The consequence is deliberate and must be visible, not silent: creating a child project widens access by inheritance, so a project's Access surface **shows inherited grants separately from its own, naming the ancestor they come from**, and project creation is audit-logged **together with the grants it inherited**. **Activating a kit at project scope is its own right** (`activate kit at project scope`), granted on the project and distinct from tenant administration, so a project owner cannot change the effective ontology on the strength of ordinary project rights.

| Term                               | Meaning                                                                                                                                                                                                                                                                                                                                                                                                                    | Primary relationships                                                           | Facing        |
| ---------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- | ------------- |
| **Role**                           | Named permission set from the tenant's role catalog. ⚠️ Not the org model's Job Role.                                                                                                                                                                                                                                                                                                                                      | Granted to memberships, teams and non-human actors via role grants — a grantee may hold **any number** of roles, one scope per grant.                    | Admin-facing  |
| **Role Grant**                     | The fact that a **membership, a team or a non-human actor** holds a role **at a scope** — tenant or project (a project-scope grant reaches the subtree, D-083). Tenant-scope grants govern control-plane administration (memberships, connectors, kits, policies, attaching shared objects) and tenant-wide surfaces. Never granted to a Person (invariant 15).                                                                                                                                                                                                                          | Grantee (membership \| team \| actor) × Role × Scope; archiving a scope suspends its grants. | Admin-facing  |
...
| **Data Policy**                    | Rule governing **the data**: what is visible, what is kept, what is redacted, what may be fed to a model. It hides rather than blocks. Sensitivity, retention, redaction, model usage **and visibility** over object types and sources. Visibility per object type: **full / stub / concealed**; shipped default = **stub**. A control-plane citizen (§3).                                                                                                                                                                                                                                                                                                                              | Constrains projections, context packages, model routing.                        | Admin-facing  |
...
| **Review Request**        | A routed ask for one or more people or teams to review something: a pull request, a document, an architecture change, a risk. It carries the routing and the state of the ask; the review itself is a Validator run and its outcome is a Validation Status. Without it, "who was asked and did they answer" lives only in a source tool. | Targets a managed object or candidate; routed to memberships or teams; produces validation results. | User-facing |
| **Approval Request**      | A routed ask for an approval **decision**, separate from the decision itself. The request is the queue item: who was asked, since when, still open or not. The **Approval** is the recorded outcome. Keeping them apart is what makes "nothing is waiting on me" answerable. | Targets a prepared action run, a candidate object or a write-back; routed to memberships or teams; resolved by exactly one Approval. | User-facing |
...
| **Safety Constraint**              | Rule governing **the act**: it stops a run rather than hiding data. Prevents unsafe automation — unauthorized write-back, sensitive-data exposure, uncontrolled model usage. *A model call falls under this and under Data Policy both, because it is an act that touches data: the Data Policy decides what the model may see, the Safety Constraint decides whether the call may happen at all.* Both cite the organization's **Policy** (§9) they enforce — a citation, never a path authorization travels. | Constrains actions, workflows, agents; enforces a Policy.                       | Admin-facing  |
...
| **Model Endpoint** | A provider-neutral point at which Studio calls a model, with its context limit, price and data policy. Its status — available, degraded, disabled — describes the access, not the model. ⚠️ Not the org's **AI Model** (§9), which is a model the organization has deployed and pays for; the same model may be both. | Selected by a Model Routing Policy; billed through AI Runs. | Admin-facing |
| **Tool** | A typed capability an agent, workflow or person may invoke through a registered endpoint. What a Model Routing Policy picks when the answer is not a model. | Invoked by agents and workflows; each invocation is audited. | Admin-facing |
| **Agent Definition** | The configuration of an AI-assisted worker Studio runs: instructions, tools, model policy, budget, approval boundaries. Definition, not execution — its runs are AI Runs (§6.3), and the thing that acts is an **Actor** (§6.1) playing an **Agent Role**. ⚠️ Not the org's **AI Agent** (§9), which is an agent deployed in the organization; the same agent may be both. | Plays agent roles; consumes model endpoints and tools; capped by a Cost Budget. | Admin-facing |
| **Model Routing Policy** | Rule selecting the model endpoint or tool per activity type, role, cost, risk or data policy — lockable by the organization.                                     | Governs AI runs; set at tenant/project scope.                    | Admin-facing  |
...
| **Cost Budget** | Cap on spend at a scope: tenant, project, workflow, agent or model. | Caps AI runs; alerts and blocks per policy. | Admin-facing |
...
- **Configuration is an object.** Every durable kit configuration is a container-owned graph object with immutable versions; a type's applied lifecycle is selected through its lifecycle configuration binding.
...
| **Mention** | An explicit reference to a person, team or actor inside a comment, message or document. Not decoration: it is what turns a passing remark into someone's inbox item, so it is recorded rather than parsed out of text each time. | Sits inside a Comment or Message; targets a Membership or Team; raises a Notification. | User-facing |
| **Conversation** | A durable, scoped session between a member and the Assistant: an ordered set of messages together with the context packages, action runs and objects it produced. Distinct from a Discussion Thread, which is people commenting on an object; a Conversation is a working session that *makes* things. Two anchored kinds ship with it: a **Document Conversation** on a page or section, and an **Inline Conversation** pinned to a precise code, diff or checklist location. | Groups messages; correlates the runs and decisions it produced (§6.6); belongs to a project. | User-facing |
| **Message** | One turn in a Conversation: typed content, author, references, token usage and any tool activity. Immutable. ⚠️ Not a Comment: a comment annotates an object, a message is a turn in a session. | Belongs to a Conversation; authored by an Actor. | User-facing |
...
- a **graph run** — a Workflow Run or Action Run over the project tree (§3.3, §6.1);
- a **control-plane decision** — installing a kit (a Kit Activation), creating a root project (§7, §3).
...
*One trace: a member types "tighten our release quality" → the Assistant proposes a sequence — *run gap analysis · draft the missing checks (Action Runs) · generate a review Workflow · surface the Findings* → the member approves it in **plan-preview** → each step runs as its existing mechanism, every effect visible and openable, attributed to the agent, write-backs gated by approval. A step that crosses into the control plane — install a kit, create a root project — is the same shape: an independently-approved governed decision in the sequence.*

## 7. Lifecycle, Packaging And Vocabulary

| Term | Meaning | Primary relationships | Facing |
|---|---|---|---|
| **Construction Lifecycle (CLC)** | The lifecycle **framework** a domain-skeleton kit defines for the containers that activate it — stages, activities, gates. **Kit-defined, not platform-defined**: Studio ships no universal stage ladder, and a non-production vertical's kit names its own lifecycle — CLC is the frame, not a mandated word. The flagship **SDLC Kit** ships the default: phases **Plan / Build / Operate**, 14 stages — Intent, Vision, Discovery, Strategy, Definition, Design, Construction, Validation, Release, Operation, Support, Intelligence, Optimization, Evolution — realizing the vision's *Product Lifecycle* (§4.5–4.6). The kit keeps its market-familiar name. | Contains lifecycle phases; phases contain lifecycle stages; stages define activities with their inputs/outputs, performed via actions (§6.1). | User-facing (configured per container by admins) |
...
| **Workflow Library** | Catalog of workflows published to a container — predefined and custom. What a member browses and runs — the **secondary door by decision**: primary invocation is NL-first + plan-preview; the library is the browsable catalog and the "what can Studio do here?" fallback. | Contains published workflows; populated by kits. | User-facing |
| **Kit** | Package of reusable delivery knowledge that **defines a domain**: **object & relation type definitions** (the ontology contribution), **terminology overrides** (the domain's own labels), **reference catalogs** (default value sets), **a Project Profile** (what a project of this domain requires: mandatory creation fields, its CLC, required connector classes, document types and their quality lamps), templates, workflows, actions, validators, policies, reference architectures, and **an optional sandboxed UI extension** (the safety constraint on it is a kernel mechanic — §6.4). Two origins: **shipped** (by Constructor) and **authored** in the organization. Which kits exist is catalog content; the entity shape is fixed here. | Packages object & relation types, workflows, actions, validators, templates, Gears building blocks; published to the Kit Catalog; **may extend another kit** (`Kit extends Kit`, base version pinned, a derived kit adds but never weakens the base's schema); installed into a **container** — the tenant, or a project and its subtree: install **registers the kit's types tenant-wide** and **activates** them (with workflows, templates, validators) in the installing container (invariant 13). Activation is itself a versioned, audited **container** decision pinning the exact kit version and what it enables; a new kit release or any permission widening requires a new explicit activation decision (kernel contract). | Admin-facing |
...
| **Project Profile** | The kit's answer to "what is a project in this domain": the **mandatory fields asked at creation**, the optional ones, which CLC applies, the required connector classes, the document types with their quality lamps and the rules that recognize them, and the default views. The universal minimum below any profile is three fields — name, goal, owner; everything else a profile declares. Kit-declared fields reach the built-in Project type as **governed external attributes**, so Project stays one type resolving to one organization-model term (invariant 1). A child project may select a different profile than its parent, but the **union** of both profiles' mandatory fields still applies (invariant 17). | Packaged by a kit (§8.1); selected by a project; applied at the level where its kit is activated. | Admin-facing |
...
| **re-value** | change a field's allowed values | add Work Item type *"chore"* | **Reference Catalog** (§7) — bound to an Object Type attribute (§3.2); edited per container, versioned + audited (§8.1) |
| **re-type** | add a new kind of object | introduce *"OKR"* | **custom Object Type** (§3.2) — still resolves to one org-model term (invariant 1); registered tenant-wide, activated per container (invariant 13, §8.1) |
| **re-structure** | bring a whole domain | your own vertical kit | **Kit** (§7) — installed & activated per container: the tenant or a project subtree (§8.1); may extend another kit; optional sandboxed UI extension (§6.4); the product promise is PC-12 (Appendix C) |

Most adaptation is the bottom two rungs — no forking, governance intact.

**The kernel is inert — kits make it a product.** Studio ships as three layers: the **kernel** (the engine — no domain opinions of its own); the **guaranteed mechanism kit-set** — **`cf.governance`** (Finding, Recommendation, Approval, Validator, Quality Gate, evidence-gating) and **`cf.ai`** (AI Run projection, meters, Cost Budget) — **non-removable**, version-pinned, auto-activated on every tenant; and **at least one domain-skeleton kit** — ontology + CLC + acting wiring — **swappable**, default = **SDLC Kit**, never removable to zero. The "never to zero" rule is **per container**: a container either activates its own domain skeleton or inherits the nearest one above it, which is what lets a project subtree run its own CLC (a research subproject under a delivery umbrella) while no container is ever left without one. The trust ramp (PC-1) and "AI cost per accepted change" (PC-6) are product guarantees carried by the mechanism kit-set — **"kit-packaged" says where a capability lives, never whether it is promised.** (OS analogy: kernel vs distribution — "no kit" means "no userland", not "no product".)
...
*Where the two sets fall short.* Neither set covers everything a model needs to say, and pretending otherwise produces synonyms nobody can tell apart. Two rules keep it honest. **One:** before inventing a predicate, check both sets — `contains` and `includes` must not both exist for the same idea. **Two:** when a relation genuinely needs a verb neither set has, it belongs to whichever set owns its subject — a Studio-native relation extends this catalogue, an organization-domain one is a request against the organization model. A kit may not quietly mint predicates for organization entities.

| Subject                                    | Predicate         | Object                                                                                        | Cardinality              | Notes                                                                                                                              |
| ------------------------------------------ | ----------------- | --------------------------------------------------------------------------------------------- | ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------- |
| Tenant                              | represents        | the Organization *(no object of its own)*                                                     | `1:1`                    | Cardinality unchanged. Service-provider topology = an **admin hierarchy of tenants** (next row), not one tenant over many orgs.                                                                               |
...
| Tenant                              | contains          | Project (root) / Org Unit / Membership / Team / Role / Connector                                  | `1:N`                    | Root projects are the tenant's top-level containers (D-085). The tenant keeps the org chart and the role catalog; a Team is never inside an Org Unit.                                            |
| Project (root) | owns | Managed Object (tree-scoped) | `1:N` | Objects whose type is tree-scoped belong to the tree that created or imported them, held by its root. A root cannot be deleted or archived while it owns objects (invariant 6b). |
| Tenant | owns | Managed Object (shared) | `1:N` | Objects whose type is shared exist once per tenant, with one owner and one history; project trees attach them. |
| Tenant                                     | owns              | Knowledge Graph                                                                               | `1:1`                    | One graph per tenant; a project tree is a scope inside it, not a container of its own graph.                                          |
...
| Project                                    | includes          | Managed Object                                                                                | `N:M`                    | By reference, within the tree; an object may be in several projects. Stored as its own record — **Inclusion**: who, when, manual or rule.                                                                                |
...
| Project                                    | contains          | Project (child)                                                                               | `1:N` optional           | Strict tree: at most one parent; a parentless project is the tree's **root** and owns its tree-scoped objects (invariant 6b). Configuration inherits down; data, scope and ownership never do (invariant 17). Reparenting inside the tree is out of MVP scope (D-080). |
| Project                                    | selects           | Project Profile                                                                               | `N:1` optional           | The kit-shipped definition of what a project of this domain requires (§7). A child may select another profile; the union of both profiles' mandatory fields still applies. |
| Membership | references | Person (shared object) | `N:1` | Person is tenant-scoped and shared, so a membership names exactly one; a person may hold one tenant membership and any number of team memberships. **Cross-plane association, not a graph Relationship** — the membership is a control-plane record, the Person a graph object. |
| Project (root) | attaches | Shared Object | `N:M` | Stored as an **Attachment**: who, when, active or revoked. Granted at tenant scope; detaching retires that tree's relations to the object (invariant 4). |
| Local Overlay | belongs to | Project (root) × Shared Object | `N:1` | The tree's own attributes on a shared object; invisible elsewhere; never changes the shared object. |
| Cross-tree Reference | belongs to | Project (root) | `N:1` | One-way, never traversed, invisible to workflows and validators (invariant 5c). |
| Membership / Team / Actor                  | granted           | Role                                                                                          | `N:M`                    | Stored as **Role Grant** (grantee × role × scope). Never granted to a Person (invariant 15).                                       |
| Team                                       | has member        | Person                                                                                        | `N:M`                    | Stored as a **Membership** with team scope, dated and with allocation. May be mirrored; role grants stay on authored memberships only. |
| Role                                       | bundles           | Permission                                                                                    | `1:N`                    | Permission = right × resource.                                                                                                     |
| Data Policy / Safety Constraint            | enforces          | Policy (the organization's document)                                                          | `N:1` optional           | **Cross-plane citation, not a graph Relationship** — the rule is a control-plane record, the Policy a managed object. It records why the rule exists and makes "which of our written policies nothing enforces" answerable; authorization never reads it. |
...
| Container *(Tenant \| Project)* | activates       | Object Type / Relation Type                                                                   | `N:M`                    | **Effective ontology is computed along the path** — tenant, ancestor projects, then the container itself. Activations only add along that path and never remove (invariant 13). |
...
| Connector | maps through | Object Mapping | `1:N` | Type-level mapping, versioned: external type to object type, state to state, and the field mappings under it. |
...
| Relation (tree-scoped) | belongs to | Project (root) | `N:1` | Endpoints are that tree's own objects or shared objects attached to it. Never joins the local objects of two different trees. |
| Relation (shared) | belongs to | Tenant | `N:1` | Both endpoints shared, origin mirrored: a source fact, not a tree's reading. Visible wherever both endpoints are attached. |
| View                                       | belongs to        | Tenant / Project / Membership                                                                 | `N:1`, exactly one       |                                                                                                                                    |
...
| Notification                               | targets           | Membership / Team                                                                                  | `N:M`                    | Computed / dispatched, never authored.                                                                                            |
...
| Cost Budget                                | caps              | AI Run (at a scope)                                                                           | `1:N`                    | Tenant / project / workflow / agent / model.                                                                                      |
...
| Kit                                 | packages          | Object Type / Relation Type / Terminology Override / **Reference Catalog** / **Project Profile** / Workflow / Action / Validator / Template / Gears / **UI Extension** *(sandboxed, §6.4)* | `1:N`                    | Terminology overrides ship labels; Reference Catalogs ship default value sets; a Project Profile ships what a project of this domain requires (§7); UI extension's safety constraint is the kernel mechanic in §6.4.                                                                                                                                   |
| Kit Catalog                                | contains          | Kit                                                                                    | `1:N`                    | Shipped + published by members; one catalog per tenant.                                                                            |
| Membership                                 | publishes         | Kit                                                                                    | `N:M`                    | Requires the `publish` permission; audit-logged.                                                                                   |
| Container *(Tenant \| Project)* | installs        | Kit                                                                                    | `N:M`                    | Registers the kit's types tenant-wide; activates them + populates the Workflow Library, templates, validators here and down the path (invariant 13). Installing at project scope requires the `activate kit at project scope` right (§5). |
| Kit                                        | extends           | Kit (base)                                                                             | `N:1` optional           | Kit composition: the base version is pinned, and a derived kit adds but never weakens the base's schema. |
| Tenant (parent)                            | distributes       | Kit → child Tenant                                                                     | `N:M`                    | **Down the admin hierarchy only**; audited, copy-only. Realized as **per-child-tenant distribution of an immutable kit manifest**, activated per-container as an ordinary `KitActivation` **within each child tenant** — orchestration over N independent activations, not a kernel cross-tenant primitive (kernel confirm C1/C2). No peer/third-party/monetized sharing.                          |
...
| Container *(Tenant \| Project)*             | edits             | Reference Catalog                                                                             | `N:M`                    | Adds/disables values on top of kit defaults; versioned + audited; kit defaults never deleted.                                     |
| Project (root)                             | defines           | Glossary Term                                                                                 | `1:N`                    | The customer's product-domain ontology, one glossary per tree (§7).                                                                |
...
2. Every managed object carries a **scope**, fixed by its type. A **tree-scoped** object belongs to exactly one **project tree**, owned by that tree's root project, and never appears in another. A **shared** object exists once per tenant, with one owner and one version history; project trees reach it through an **Attachment**, never by copying it. Inclusion into projects is by reference and never copies.
3. A source record maps to **at most one managed object per project tree**; mirrored and linked objects preserve source references and provenance.
4. Removing a tree-scoped object, or detaching a shared one, **retires that tree's relations to it** — and only that tree's. Other trees attached to the same shared object are untouched.
5. Every relation carries a **scope**. A **tree** relation belongs to exactly one project tree; its endpoints are that tree's own objects or shared objects attached to it, and it never joins the local objects of two different trees. A **shared** relation joins two shared objects, is a mirrored source fact, and is visible wherever both endpoints are attached.
5a. **Attaching is granted, not taken.** A project tree reaches a shared object only through an Attachment created at tenant scope, and it cannot discover a shared object it has not been granted.
5b. **Access never crosses through a shared object.** Attaching an object grants nothing about what other project trees have linked to it.
5c. **A Cross-tree Reference is not a relation.** Traversal always stops at it, workflows and validators never see it, and it creates nothing in the target tree.
6. Access never propagates along relations; objects outside a viewer's projection render per their type's Data Policy visibility level — full / stub / concealed, shipped default **stub**.
6a. **No unbounded path to the graph exists.** Every traversal, search and API path is bounded by the viewer's projection: their project trees' own objects plus the shared objects attached to them.
6b. Every object has an owner: the **root project** of its tree when it is tree-scoped, the tenant when it is shared. There are no ownerless objects. The scope handle is the root project's identity, which is stable and never reused; a **root project cannot be deleted or archived while it owns objects**, and no project ever moves between trees (D-080, D-085).
6c. A **Local Overlay** is visible only in the project tree that wrote it and never alters the shared object it annotates.
...
8. Organization-model entities that are **reified relationships** — relationships stored as their own records with attributes (Employment, Position Allocation, Reporting Line, Responsibility Assignment, Assignment, Competency possession, Dependency, Handoff) — arrive as **managed objects**, never as bare relations: their dates and attributes must survive. **Team Membership is the one exception**: it is realized as a Membership with team scope (§3.1), which already carries its dates and allocation.
...
13. The Object/Relation Type registry is **tenant-wide**; a **container** — the tenant or a project — **activates** a subset. A container's **effective ontology is computed along its path**: the union of the activations at the tenant, at its ancestor projects and at itself. **Activation along the path only adds** — a lower container may activate more, never deactivate what a higher one activated, which is why objects of an activated type can never be orphaned by a container's own configuration. Where the merged thing is a single value rather than a set (a label, a reference-catalog value, a naming convention) the **nearest ancestor shadows** the ones above it; invariant 11 still limits overrides to labels. A kit install registers the kit's types at the tenant and activates them in the installing container; deactivating never deletes a type that has instances. *(Kernel dependency: activation at project scope and path-computed effective ontology need kernel confirmation, class C1/C2; until then this is the model's target state.)*
...
15. Authorization flows through **control-plane records only** — a Membership and its Role Grants — never through graph objects. A **Person is never a grantee**. A source system may create and change a Person and a team membership; it may never create or change a **tenant** membership or a Role Grant, so no sync can hand out access. Only a tenant membership carries the right to enter Studio.
...
17. **The project tree carries configuration, never data.** A project has at most one parent; a project with no parent is a **root**, and the root owns the tree's tree-scoped objects (invariant 6b). Inheritance down the tree covers a closed list of configuration — naming convention, template set, Project Profile, required creation fields, validator and lamp sets — and never objects, scope or ownership. A parent's included objects are the **computed union** over its subtree, by reference, never copied (invariant 2). A **project-scope role grant reaches the whole subtree** (§5, D-083), so the Access surface must show inherited grants and their source ancestor, and project creation is audit-logged with the grants it inherited. Where a subtree activates its own domain skeleton its CLC differs from its parent's, and an aggregate over a subtree with mixed CLCs may sum only **stage-agnostic** quantities — findings, risks, gate status, cost, and each child's progress against its own CLC — never a shared stage.

## 9. Organization-to-Studio Mapping

First-pass mapping of the most important organization entities — every root domain plus the Work Management layer. The rest of the inventory is mapped on demand. Rows the organization model now tiers **Mentioned or Secondary** (most Commercial, External Dependencies, Strategy finance, SLO/Operational Metric) are kept here for completeness but marked *(on-demand)* — shown when needed, not managed first. Typical sources are examples, not commitments. **Division of labor with the glossary:** the glossary's divergent-terms index disambiguates shared *words*; this table maps *entities* — where a word collides, the guard lives in the glossary and rows here only point at it (⚠️). Studio-side targets are **object types registered tenant-wide**. Most are **kit-owned**: they arrive with a domain-skeleton kit and leave with it. A few are **built-in**, because every vertical needs them whatever the kit: people, teams, competencies, projects, objectives, work items and the source-control objects a mirrored estate always has. A built-in type that a vertical does not use simply carries no objects; it is not a reason to fork the product. Which supplier a type has is recorded per type (§3.2). **Import is per project tree:** each importing tree mints its own objects, each with its own source links — nothing here implies a shared cross-tree object.

| Organization entity (domain) | Studio object | Typical sources | Mirrored / authored | Where a member meets it |
|---|---|---|---|---|
| Organization (Organizational Structure) | represented by the Tenant, with no object of its own | admin setup, directory | — | tenant administration |
| Organizational Unit (Organizational Structure) | Organizational Unit *(the org chart: division, department, business unit, geography, cost center; hierarchical, with a leader)* | HRIS, directory | mirrored | org chart, people directory |
| Person (Organizational Structure) | Person — a **shared** object | HRIS, directory, Git/tracker accounts | mirrored | people directory, ownership panels *(one Person per tenant; project trees attach it, and a tenant Membership carries the right to use Studio — §3.1)* |
| Team (Organizational Structure) | Team, one entity: cross-functional, never inside an Org Unit; carries an acquisition mode (§3.1) | directory, Git groups, tracker teams | mirrored or authored | team page, ownership panels |
...
| Product Line (Product) | Product Line | product catalog, docs | mirrored or authored | the tree's product scope *(Studio has no Product Portfolio object; a cross-line view is built from tenant-wide aggregates plus the shared objects the lines have in common)* |
...
| Repository (Software Estate) | Repository — a **shared** object | GitHub, GitLab | mirrored | repository browser |
| AI Model / AI Agent (Software Estate) | AI Model, AI Agent | model registry, agent platform | mirrored | AI estate & cost views |
| Commit / Pull Request (Delivery) | Commit — a **shared** object; Pull Request *(high-volume)* | Git platform | mirrored | change history, traceability |
...
| Project (Work Management) | **Project** — the same entity: mirrored from trackers or authored in Studio; Studio adopts it as a working project (adds scope + workflows) and lets projects **nest into a tree** (invariant 17) | tracker, project-management tool | mirrored or authored | project overview; working projects |
...
```text
Start: a fresh Tenant — one root Project ("Backup & Recovery"), one child Project inside it.
  ...
  -> The admin installs the SDLC Kit and the Product Management Kit: their Object and
     Relation Types register at the tenant and activate in the root project; the Workflow
     Library, templates and validators fill in.
  -> Model routing is locked; members are invited; roles granted at the root project's scope.

Output: a ready project tree — a live Knowledge Graph mirroring the organization's tools,
...
### 10.4 A portfolio lead looks across lines (the per-tree boundary, honestly)

```text
Start: Acronis now runs two project trees — "Backup & Recovery" and "Cyber Cloud".
       A security lead holds a tenant-scope grant.
  -> The lead opens the cross-line summary: scalar aggregates per tree —
     open critical findings, write-backs pending approval, AI cost per accepted
     change — side by side. This is the per-tree layer (PC-7).
  -> She asks "show me every project tree where engineer X can write to production."
     X is a shared object — one Person for the whole tenant — so the question has
     one subject. The answer lists the trees that attached X and the grants
     her tenant-scope projection lets her see; what each line has linked to X
     stays inside that line (invariant 5b).
  -> Drill-down into a line opens that tree's own view, under that
     tree's projection. Aggregates cross the boundary; a line's relations
     to X do not.

Output: a cross-line view over one subject, with each line's own reading of
        that subject still private to it.
```
...
- **D-062 — a few domain types are built-in, not kit-owned (2026-08-03).** §7 says the kernel is inert and kits make it a product, which reads as "every domain type ships in a kit". In practice people, teams, competencies, projects, objectives and work items are needed by every vertical, and putting them in a kit would mean that kit is always installed, which makes it the kernel under another name. They stay built-in. A vertical that does not use a built-in type simply has no objects of it. The promise §7 makes is about *swapping the domain skeleton*, not about emptying the core.
...
- **D-058 — revised 2026-08-03: identity follows the type's scope; two layers.** Previously identity was container-local for everything and a third layer of Citizen Stand-ins existed. Now: **control-plane records** (Membership, Role Grant) carry authorization and never the graph; **graph objects** are shared or tree-scoped by type (re-based onto the root project by D-085). Stand-ins are gone, because a Person is an ordinary shared object that authorization simply never reads. Supersedes D-017.
- **D-059 — superseded 2026-08-03 by the shared scope (§3.1.1).** It previously specified cross-line summaries built above the working containers via correlation IDs and correspondence records, never one object across lines. The product now carries a **shared scope inside the tenant**: source-living and frozen-produced things are one object tenant-wide, and cross-line questions have one subject. The correlation approach stays the fallback only if the platform work behind the shared scope is refused.
...
| Project (root) | created → active → archived *(archiving suspends its role grants; blocked while the root owns objects, invariant 6b)* |
...
| Membership | invited → active → deactivated |
| Team | created (blank or seeded from an org Team) → active → archived *(archiving suspends its role grants)* |
| Kit | authored / shipped → published (in the Kit Catalog) → installed (per container) → archived |
...
- **PC-5 Traceability.** Within a project tree, every accepted change is traceable end to end — intent → requirement → code → test → release — pinned to exact versions.
...
- **PC-7 Portfolio, honestly phased.** Cross-line portfolio and compliance views are committed but phased: per-tree summaries and roll-up numbers ship now; drilling those summaries down to the individual objects across lines is a later phase. We say which is which rather than implying it all works today.
- **PC-8 Discoverable by default.** Inside a project tree, an object a member can't fully open still shows as a stub — it exists, with a type and a label — by default; sensitive types can be hidden entirely. Access never leaks along relationships.
...
## Appendix D. Working across project trees — requirements (R1–R10)

*The detailed requirements summarized in §3.1.1 — what the product must let people do, deliberately without choosing how. The test that decides everything below: **a thing can be shared exactly when there is one of it.** Things that live in a source (a person, a repository, a commit) and frozen-produced things (a build, a release) are shared: one object, one history, one owner, editable through governed runs because there is nothing to fork. Editable authored content (a PRD, a competitor review) is tree-scoped and crosses trees only as a copy — two editors on "the same" document would diverge.*

- **R1 Working context.** A member works in a project tree and sees only what it includes — no noise from other lines.
- **R2 Shared things exist.** One real entity can be used in several trees without manual duplication.
- **R3 One shared thing, not copies.** When the same source-living or frozen-produced thing is used in two trees it is one shared object — a change visible everywhere, one history, one owner. **Required** (§3.1.1); the kernel work that carries it is tracked in the decision register.
- **R4 Cross-cutting view.** A security / operations / portfolio lead gets a summary across trees, and drills from it into a shared object, which is one subject tenant-wide. What each line has linked to that object stays inside that line (invariant 5b).
- **R5 Hard wall on demand.** A separate mode for genuinely isolated trees (regulatory, service-provider client, trade secret). Trees are already walled — no work crosses between them; concealing even a walled tree's *existence* from org-wide surfaces *(needs a kernel change)*.
...
- **R7 Private by default; copy is the only move for tree-scoped things.** A tree-scoped object is visible only in its tree and reaches another one only as an independent copy (R8). Sharing applies to shared-scoped types only, and only within one tenant: there is no shared object across tenants.
- **R8 Two operations, by scope.** A tree-scoped object (a PRD, a competitor review) travels to another tree **only as an independent copy** — its own life from then on. A shared-scoped thing (a person, a repository, a commit, a build, a release) is attached rather than copied. Build-home rule: a build's home is the tree whose run produced it; two trees building the same repo independently make two builds, and each is shared under its own identity.
- **R9 Rights.** For authored content, each copy is owned by its tree and edited per grants. For a shared non-edited thing, ownership stays with its home or source.
- **R10 Org-level commons, minimal.** The only truly org-wide authored things — company Vision, Mission, top-level Objective — live in one designated **org-commons root project**, readable across the org by a grant on it. They stay ordinary tree-scoped objects; org-wide Policy is control-plane, not a graph object. Nothing else is hoisted.

*Where these stand: R1, R2, R5 (walls), R7, R9 and R10 hold on today's platform. **R3, R4 and R8 are now required**, and the shared scope they rest on needs platform work: shared identity is tenant-wide, which today's kernel does not carry. That work is tracked in the decision register alongside the clauses still tagged **(needs a kernel change)** — existence concealment (R5) and one-way reversibility (R6) — and now also the re-basing of the second scope tier from a workspace record onto the **root project's identity** (D-085). The model states the target; the platform follows.*
...
| **Project** | name, goal, owner | editable |
| | parent project | editable-once (no reparenting in MVP, D-080) |
| | selected Project Profile | editable-once in practice (changing it re-asks the profile's mandatory fields) |
| | progress, risks, included-object counts, subtree aggregates | system (computed) |
| | owns the tree's tree-scoped objects *(root only)* | system (invariant 6b; blocks delete/archive while non-empty) |
| | inherited configuration (naming, templates, required fields) | read-only here, editable at the ancestor that owns it |
