...
flowchart LR
  ...
  Graph -->|"contains (shared or workspace-scoped)"| MO
...
| ------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------ |
| **Tenant**    | The instance of Studio serving one Organization (**1:1** — unchanged). *Acronis = one tenant.* Commercial packaging (subscription plan, account policies) is tenant configuration, not domain entities. A tenant may sit under a **parent tenant** in a control-plane **admin hierarchy**: a service provider's parent tenant administers N child tenants, each still 1:1 with its own organization — the hierarchy governs *management*, never data (each tenant's objects stay isolated). Kernel already supports this (`parentTenantId`). | Contains workspaces, org units, memberships, teams, roles and connectors (any number of each; the tenant itself is one per organization); keeps the **control-plane registries** — memberships, teams, roles, policies, connectors, kits, audit — the tenant-wide **type** registry (invariant 13), and the single **Knowledge Graph**, in which shared objects are tenant-scoped and workspace objects belong to their workspace. **May administer child tenants** (`Tenant → administers → Tenant`, 1:N, control-plane only, deployment-gated). | Admin-facing |
| **Workspace** | Studio's working context, bounded by **one purpose** — typically a product line, or several products linked to each other; may span departments and teams; may be personal.                                                                                                                                                                                                                                                                                                                                                                  | Belongs to the tenant (a tenant runs **any number** of workspaces); **owns** the objects whose type is workspace-scoped and **attaches** the shared ones it is granted (§3.1.1); is a scope inside the tenant's single Knowledge Graph rather than the owner of a graph; hosts any number of projects, views, workflows, automation.                                                                                                                  | User-facing  |
...
**One graph, two scopes, defined once here** *(referenced throughout the doc)*: the tenant has **one Knowledge Graph**, and every object and every relation in it carries a **scope**. A **shared** object lives at tenant level and workspaces *attach* it; a **workspace** object belongs to exactly one workspace. Scope is not chosen per object: it is declared **by the object's type**, and a kit declares it (§3.2). Where the doc says "a shared object," it means an object whose type is declared shared.

Isolation therefore rests on **filtering, not on structure**: there is no graph above the workspace to wall things off, so every traversal, search and API path is bounded by the viewer's projection, and no unbounded path to the graph exists (invariants 6a–6c). This is a deliberate trade: one graph is simpler to reason about and to implement, and the guarantee moves into rules that must be owned and tested.

**What is shared, and what stays local.** Things that live in a source and that Studio mirrors rather than authors are shared: a **Person**, a repository, a commit, a branch, a build, a release, a software system, a service. One object per tenant, one history, one owner. Editable authored content — a PRD, a competitor review, a decision — stays **workspace**-scoped: it belongs to the workspace that authored it and reaches another workspace only as an independent copy, because two editors of "the same" document would diverge.
...
**A workspace may add its own reading on top of a shared object** — a Local Overlay (§3.2): attributes that belong to that workspace, invisible to others, that never change the shared object itself.

The requirements (R1–R10), and which clauses still need a kernel change, are in **Appendix D**.

**Studio supports service-provider / MSP topologies** (VISION §6.4 "Service providers: Core focus"): a **parent tenant administers N child tenants**, each still 1:1 with its own organization — one provider console over many client tenants, with the provider able to push its kits down the hierarchy (§7). The **tenant admin hierarchy** (§3.1 Tenant) that enables this is a *different axis* from cross-workspace sharing and must not be confused with it: it is **control-plane only** — the hierarchy governs *management*, never data — and it creates **no** shared objects, no shared identity and no cross-tenant graph. Sharing is a **within-tenant** mechanism only: a shared object is shared across the workspaces of one tenant and never across tenants. The only sanctioned cross-tenant move is a parent **distributing a kit** (definition, copy-only) down the hierarchy (§7).

### 3.2 Objects and relations

| Term                | Meaning                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Primary relationships                                                                                                                            | Facing        |
| ------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ | ------------- |
| **Knowledge Graph** | The tenant's single home for the facts: every managed object and every relation, each carrying a **scope** (§3.1.1). One graph per tenant, not per workspace — a workspace is a *scope within it*, not a container of its own graph. A graph is a **system model, not a UI**: members see views; a graph diagram is just one view. | Belongs 1:1 to a tenant; queried by views, workflows and automation, always through the viewer's projection. | System-facing |
| **Managed Object**  | Studio's representation of one real entity. It **is the kernel `Object` metatype seen at the product layer**, with acquisition/provenance/sync added; the name keeps "Managed" because bare "object" is used generically throughout. *(STUDIO_VISION calls this "artifact"; kernel/ARCH say "Object" — same entity, see glossary alias table.)* Every object carries a **scope**: **shared** (one per tenant, workspaces attach it) or **workspace** (belongs to exactly one). Scope comes from the object's **type**, never from a per-object choice. A shared object has one owner and one version history, so editing it is allowed and forking it is not (§3.1.1). Everything else it carries — below this table. | Typed by an Object Type, which fixes its scope; assembled from source records via identity mapping (authored objects have none); attached to workspaces when shared, owned by one when not; included into projects. | User-facing |
| **Relation**        | A typed, directed link between exactly two managed objects — *"PR **resolves** Work Item"*. Like an object it carries a **scope**. A **shared** relation joins two shared objects and is a mirrored source fact (a commit belongs to a repository); it is visible to every workspace that has both endpoints attached. A **workspace** relation belongs to exactly one workspace and may join that workspace's own objects, an object and an attached shared one, or even two shared ones — the last being that workspace's own reading, invisible to others. Carries an **origin** (imported, or asserted / inferred) plus the same provenance and confidence as an object. | Instance of a Relation Type; scoped shared or to one workspace. | User-facing |
| **Object Type**     | The blueprint of one kind of object: what a Requirement, a PR or a Release *is* — its expected attributes, which relations it may enter, and **whether its objects are shared or workspace-scoped** (§3.1.1). Four rules: every domain-facing type stands for **exactly one organization-model term**; a type may **specialize** another (Metric-by-domain, Decision subtypes); a type declares its **scope**, and a kit ships that declaration; **one registry per tenant, each workspace activates its subset** — the workspace's *effective ontology* (invariant 13). Types come from three suppliers — **built-in**, **kit install**, **custom** — recorded per type. Customers may extend kit types with governed external attributes without forking the type — declared by kits, activation-gated, never weakening the base schema (kernel contract). A typed attribute may be **constrained by a Reference Catalog** (§7) — an editable value set, not a fixed schema enum — when its allowed values are an operational choice. | Types managed objects and fixes their scope; defines expected attributes and allowed relation types; registered at the tenant; activated per workspace. | Admin-facing |
...
| **Attachment**      | The record that a workspace uses a shared object: who attached it, when, and whether it is still active. Attaching is a **tenant-scope right** (§5) — a workspace cannot help itself to a shared object it was not granted. Detaching retires that workspace's own relations to the object and touches no other workspace. | Pairs one workspace with one shared object; created and revoked at tenant scope; audit-logged. | Admin-facing |
| **Local Overlay**   | A workspace's own reading of a shared object: attributes that belong to that workspace alone. *"In our line this repository is release-critical."* Invisible to other workspaces, and it never changes the shared object. | Belongs to one workspace and one shared object; edited per that workspace's grants. | User-facing |
| **Cross-workspace Reference** | A deliberately weak pointer from an object in one workspace to an object in another, for cross-line dependencies a shared object cannot express (*"our release depends on their component"*). **It is not a relation**: traversal always stops at it, workflows and validators do not see it, the far side renders at its type's Data Policy visibility level, and nothing appears in the target workspace. | Belongs to the workspace that created it; one-way; never traversed (invariant 5c). | User-facing |

**What every managed object carries:**

- **Scope** — **shared** (one object per tenant, workspaces attach it) or **workspace** (belongs to exactly one). Fixed by the object's type, never chosen per object.
...
*The same engineer appears as a Jira user, a GitLab account and an HRIS row — three source records. Identity mapping declares them **one Person object for the tenant** — Person is a shared type (§3.1.1), so a second workspace importing the same sources attaches that same object rather than minting its own.*
...
| **Identity Mapping** | Rule or confirmed fact that several source records are the same real entity. Studio proposes matches (e-mail, linked accounts, names); an administrator confirms uncertain ones and can split wrong merges; every merge/split is audit-logged. | Merges source records into one managed object, at the scope its type declares: one per tenant for a shared type, one per workspace otherwise. Object identities themselves are never merged, split or reused. | Admin-facing |
...
| **Role Grant**                     | The fact that a **membership, a team or a non-human actor** holds a role **at a scope** — tenant, workspace or project. Tenant-scope grants govern control-plane administration (memberships, connectors, kits, policies, attaching shared objects) and tenant-wide surfaces. Never granted to a Person (invariant 15).                                                                                                                                                                                                                          | Grantee (membership \| team \| actor) × Role × Scope; archiving a scope suspends its grants. | Admin-facing  |
...
| **Model Routing Policy** | Rule selecting the model endpoint or tool per activity type, role, cost, risk or data policy — lockable by the organization.                                     | Governs AI runs; set at tenant/workspace scope.                  | Admin-facing  |
...
| Tenant                              | contains          | Workspace / Org Unit / Membership / Team / Role / Connector                                       | `1:N`                    | The tenant keeps the org chart and the role catalog; a Team is never inside an Org Unit.                                            |
| Workspace | owns | Managed Object (workspace-scoped) | `1:N` | Objects whose type is workspace-scoped belong to the workspace that created or imported them. |
| Tenant | owns | Managed Object (shared) | `1:N` | Objects whose type is shared exist once per tenant, with one owner and one history; workspaces attach them. |
| Tenant                                     | owns              | Knowledge Graph                                                                               | `1:1`                    | One graph per tenant; a workspace is a scope inside it, not a container of its own graph.                                          |
...
| Workspace | attaches | Shared Object | `N:M` | Stored as an **Attachment**: who, when, active or revoked. Granted at tenant scope; detaching retires that workspace's relations to the object (invariant 4). |
| Local Overlay | belongs to | Workspace × Shared Object | `N:1` | The workspace's own attributes on a shared object; invisible elsewhere; never changes the shared object. |
| Cross-workspace Reference | belongs to | Workspace | `N:1` | One-way, never traversed, invisible to workflows and validators (invariant 5c). |
...
| Relation (workspace-scoped) | belongs to | Workspace | `N:1` | Endpoints are that workspace's own objects or shared objects attached to it. Never joins the local objects of two different workspaces. |
| Relation (shared) | belongs to | Tenant | `N:1` | Both endpoints shared, origin mirrored: a source fact, not a workspace's reading. Visible wherever both endpoints are attached. |
| View                                       | belongs to        | Tenant / Workspace / Project / Membership                                                     | `N:1`, exactly one       |                                                                                                                                    |
...
2. Every managed object carries a **scope**, fixed by its type. A **workspace-scoped** object belongs to exactly one workspace and never appears in another. A **shared** object exists once per tenant, with one owner and one version history; workspaces reach it through an **Attachment**, never by copying it. Inclusion into projects is by reference and never copies.
...
4. Removing a workspace-scoped object, or detaching a shared one, **retires that workspace's relations to it** — and only that workspace's. Other workspaces attached to the same shared object are untouched.
5. Every relation carries a **scope**. A **workspace** relation belongs to exactly one workspace; its endpoints are that workspace's own objects or shared objects attached to it, and it never joins the local objects of two different workspaces. A **shared** relation joins two shared objects, is a mirrored source fact, and is visible wherever both endpoints are attached.
5a. **Attaching is granted, not taken.** A workspace reaches a shared object only through an Attachment created at tenant scope, and it cannot discover a shared object it has not been granted.
5b. **Access never crosses through a shared object.** Attaching an object grants nothing about what other workspaces have linked to it.
5c. **A Cross-workspace Reference is not a relation.** Traversal always stops at it, workflows and validators never see it, and it creates nothing in the target workspace.
...
6a. **No unbounded path to the graph exists.** Every traversal, search and API path is bounded by the viewer's projection: their workspaces' own objects plus the shared objects attached to them.
6b. Every object has an owner: a workspace when it is workspace-scoped, the tenant when it is shared. There are no ownerless objects.
6c. A **Local Overlay** is visible only in the workspace that wrote it and never alters the shared object it annotates.
...
| Person (Organizational Structure) | Person — a **shared** object | HRIS, directory, Git/tracker accounts | mirrored | people directory, ownership panels *(one Person per tenant; workspaces attach it, and a tenant Membership carries the right to use Studio — §3.1)* |
...
| Product Line (Product) | Product Line | product catalog, docs | mirrored or authored | workspace product scope *(Studio has no Product Portfolio object; a cross-line view is built from tenant-wide aggregates plus the shared objects the lines have in common)* |
...
Start: Acronis now runs two workspaces — "Backup & Recovery" and "Cyber Cloud".
  ...
  -> She asks "show me every workspace where engineer X can write to production."
     ...
     one subject. The answer lists the workspaces that attached X and the grants
     her tenant-scope projection lets her see; what each line has linked to X
     stays inside that line (invariant 5b).
  -> Drill-down into a line opens that workspace's own view, under that
     workspace's projection. Aggregates cross the boundary; a line's relations
     to X do not.
...
- **D-058 — revised 2026-08-03: identity follows the type's scope; two layers.** Previously identity was workspace-local for everything and a third layer of Citizen Stand-ins existed. Now: **control-plane records** (Membership, Role Grant) carry authorization and never the graph; **graph objects** are shared or workspace-scoped by type. Stand-ins are gone, because a Person is an ordinary shared object that authorization simply never reads. Supersedes D-017.
- **D-059 — superseded 2026-08-03 by the shared scope (§3.1.1).** It previously specified cross-line summaries built above workspaces via correlation IDs and correspondence records, never one object across lines. The product now carries a **shared scope inside the tenant**: source-living and frozen-produced things are one object tenant-wide, and cross-line questions have one subject. The correlation approach stays the fallback only if the platform work behind the shared scope is refused.
...
- **R3 One shared thing, not copies.** When the same source-living or frozen-produced thing is used in two workspaces it is one shared object — a change visible everywhere, one history, one owner. **Required** (§3.1.1); the kernel work that carries it is tracked in the decision register.
- **R4 Cross-cutting view.** A security / operations / portfolio lead gets a summary across workspaces, and drills from it into a shared object, which is one subject tenant-wide. What each line has linked to that object stays inside that line (invariant 5b).
...
- **R7 Private by default; copy is the only move for workspace-scoped things.** A workspace-scoped object is visible only in its workspace and reaches another one only as an independent copy (R8). Sharing applies to shared-scoped types only, and only within one tenant: there is no shared object across tenants.
- **R8 Two operations, by scope.** A workspace-scoped object (a PRD, a competitor review) travels to another workspace **only as an independent copy** — its own life from then on. A shared-scoped thing (a person, a repository, a commit, a build, a release) is attached rather than copied. Build-home rule: a build's home is the workspace whose run produced it; two workspaces building the same repo independently make two builds, and each is shared under its own identity.
